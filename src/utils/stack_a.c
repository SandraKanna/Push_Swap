...
// static int	head_1(int next, int count, t_node **set_list)
{
	// int	mid;
	int	prev;
	...
	else if ((next != 2 && next != 3 && prev != 5)
		|| next == 5)
		sa(set_list);
	else if (next == 3 && last == count)
		ra(set_list);
...
static int	head_4(int next, int last, int count, t_node **set_list)
{
	...
	if (prev == 3 || last == 5)
		rra(set_list);
	else if ((last == 3 && count != 5) || next == 5)
		ra(set_list);
	else if ((last != 5 && next == 3))
		sa(set_list);
	return (is_stack_sorted(*set_list));
}
